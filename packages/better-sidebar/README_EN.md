...
<div align="center">
  ...
  <a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="Supported DSH versions (v0.18.1-alpha.4 alpha track): 0.1.2-alpha.3" src="https://img.shields.io/badge/DSH-0.1.2--alpha.3_(alpha%20track)-4d6bfe" /></a>
...
**Supported DSH versions**: <a href="https://www.npmjs.com/package/@deepseek-ai/dsh?activeTab=versions"><img alt="Supported DSH versions (v0.18.1-alpha.4 alpha track): 0.1.2-alpha.3" src="https://img.shields.io/badge/DSH-0.1.2--alpha.3_(alpha%20track)-4d6bfe" /></a> · full release history on the [Releases](https://github.com/omdsh-dev/DSH-better-sidebar/releases) page

### v0.18.1-alpha.4

- ⚡ **Workspace-aware clangd compilation database selection**: the clangd launcher starts from the current session `cwd`, checks the root, `build/`, and `out/build/` first, then performs a depth- and directory-bounded search before passing the selected directory through `--compile-commands-dir`; common C++ template/generated-header extensions `.inc/.inl/.ipp/.tpp` are routed to clangd too.
- 📊 **Language-service status bar**: the bottom of each code editor shows project-config detection, server startup/parsing, navigation-ready latency, or failure; common C++, Python, and TypeScript project files are identified.

### v0.18.1-alpha.2

- 🧭 **Navigation controls moved to the tab strip**: Back/Forward are fixed at the right of the top sidebar tab strip, immediately before the panel-layout controls, with reserved room for all four buttons.

### v0.18.1-alpha.1

- 🧭 **VS Code-style code navigation**: Cmd/Ctrl-click records the source file and exact cursor position. Back/Forward buttons at the right of the sidebar tab strip traverse that timeline, and a new jump after Back clears the old Forward branch.
